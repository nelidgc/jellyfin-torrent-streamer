...
`rebuild --dry-run` не изменяет `.strm`, реестр или TorrServer. Он строит план из реестра (и доступных метаданных TorrServer) и атомарно сохраняет его в `paths.state\rebuild-preview.json`; другой файл можно указать через `--report`. Метаданные TorrServer запрашиваются одним `list`, без открытия каждого торрента. В отчёте есть старый/новый путь, hash, индекс, источник названия и действие. Обычный `rebuild` сначала сохраняет резервную копию `imports.json`, повторяет проверку плана, записывает ссылки непосредственно из проверенного плана и отказывается продолжать при ошибках, выходе за медиатеку, риске перезаписи пользовательских файлов или изменении количества управляемых записей.

### Удаление отдельного торрента

Удалять только карточку в Jellyfin, запись в веб-интерфейсе TorrServer или файлы `.strm` недостаточно: запись остаётся в `data\state\imports.json`, а архивный `.torrent` — в `data\processed`, поэтому следующий `rebuild` восстанавливает ссылки. Сначала найдите короткий hash:

```powershell
# Торренты, у которых часть или все STRM уже удалены вручную
node .\torrent-jellyfin.mjs list --missing

# Разные раздачи, претендующие на одно имя фильма или эпизода
node .\torrent-jellyfin.mjs list --duplicates
```

Проверьте план и только затем подтвердите удаление:
...
Hash-префикс должен содержать минимум 8 символов и однозначно определять запись. Без `--yes` команда всегда работает как безопасный предпросмотр. При подтверждении она удаляет торрент из реестра и TorrServer вместе с его одноразовым кэшем, удаляет только проверенные управляемые `.strm`, сохраняет резервную копию `imports.json`, а исходный `.torrent` перемещает в `data\removed`. Изменённый пользователем `.strm`, символическую ссылку или путь вне медиатеки команда не трогает и полностью останавливает операцию.

Работающий сервис подхватывает изменение реестра автоматически. После удаления запустите сканирование медиатеки в Jellyfin. Для восстановления перенесите нужный `.torrent` из `data\removed` обратно в `data\inbox`.
...
| Удалённые торренты для восстановления | `data\removed` | создаётся автоматически рядом с `paths.processed` |
...
`connectionsLimit: 100` и `readerReadAheadPercent: 100` дают TorrServer больше источников и окно загрузки вперёд для тяжёлых 4K-потоков. `torrentDisconnectTimeoutSeconds: 120` сохраняет найденных пиров между короткими повторами и перемотками, но не удерживает случайно открытые торренты активными десять минут. Эти параметры уменьшают вероятность пауз, но не могут компенсировать раздачу, которая реально отдаёт медленнее битрейта видео.

Обычный `HEAD` без Range обслуживается по сохранённому размеру файла и не активирует торрент во время сканирования медиатеки. `gateway.positionPrebufferBytes` включает предварительное чтение с запрошенной позиции перед открытым `GET Range`: `0` отключает эту защиту, а `67108864` создаёт стартовый буфер 64 МиБ. Закрытые диапазоны, `HEAD` и запросы хвоста файла не прогреваются. Это увеличивает время запуска холодного торрента, но предотвращает начало обречённой сессии без запаса данных. За последующую загрузку вперёд отвечает потоковый reader TorrServer: `preloadPercent` остаётся его внутренней настройкой, а `responsiveMode: true` закрепляет режим с минимальной задержкой ожидания pieces.

Фоновый прогрев метаданных по умолчанию отключён (`metadataWarmupBytes: 0`, `metadataWarmupRecentTorrents: 0`), потому что пакет сериала умножает объём прогрева на число эпизодов. При необходимости его можно включить вручную: он читает заданный объём с начала и конца каждого видео только при отсутствии пользовательского потока и отменяется при начале просмотра.

`uploadRateLimit` и `downloadRateLimit` задаются в KiB/s: `null` сохраняет текущее значение TorrServer, `0` снимает ограничение, положительное целое задаёт лимит. `disableUpload: null` также ничего не меняет. Не включайте `disableUpload: true` без понимания последствий: закрытые трекеры могут требовать раздачу и применять санкции за низкий рейтинг. `disableUpnp: false` разрешает TorrServer запросить проброс фиксированного peer-порта у домашнего маршрутизатора; `install.ps1 -ConfigureFirewall` создаёт для этого порта узкие TCP/UDP-правила Windows. `watch.peerCheckMs` задаёт длительность фоновой проверки подключённых пиров после импорта; наличие только `totalPeers` не считается установленным соединением.
...
Во время воспроизведения шлюз пишет события `Torrent stream response`, `Torrent stream stalled`, `Torrent stream resumed` и `Torrent stream finished`. В событии ответа фиксируются фактические downstream/upstream-статусы и время ответа; в событии зависания есть скорость загрузки, количество активных пиров/сидов, Range и время без данных. Итоговое событие содержит время первого байта и среднюю скорость выдачи. Обычные клиентские `ECONNRESET`/`EPIPE` молча закрываются, а настоящие ошибки HTTP-протокола остаются в журнале.
