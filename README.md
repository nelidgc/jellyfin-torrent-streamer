...
Название выбирается из двух источников: метаданных торрента и имени `.torrent`-файла. Транслитом может оказаться любой из них:

```text
.torrent "[rutor.is]CHerepashki-nindzya.2014"  метаданные "Черепашки-ниндзя.2014.avi"
.torrent "[GTorrent.cc]_Черепашки-ниндзя"      метаданные "Cherepashki.Ninz9.2014.avi"
```

Транслит всегда состоит только из ASCII, а название, вместо которого он стоит, — нет. Поэтому при наличии не-ASCII букв ровно с одной стороны выигрывает эта сторона; если такого признака нет, берутся метаданные торрента. Когда обе строки латиницей и одна из них транслит, признака не существует — такой случай решается только вручную.
...
Настройки раздачи и проверки живости торрента:

| Настройка | По умолчанию | Назначение |
|---|---|---|
| `torrServer.uploadRateLimit` | `null` | Предел скорости раздачи, передаётся в поле TorrServer `UploadRateLimit`. `null` — не трогать настройку, `0` — без ограничения. |
| `torrServer.downloadRateLimit` | `null` | То же для `DownloadRateLimit`. Ограничение бережёт канал, но замедляет наполнение кэша и старт просмотра. |
| `torrServer.disableUpload` | `null` | `true` полностью выключает раздачу (`DisableUpload`). На закрытых трекерах это нарушает правила и ведёт к бану. |
| `watch.peerCheckMs` | `10000` | Сколько ждать появления пиров после импорта, прежде чем записать предупреждение в журнал. `0` — не проверять. |

Настройки TorrServer записываются только при реальном отличии, поэтому общий с чем-то ещё TorrServer не перетирается при каждом старте.
...
`npm test` работает на подставном TorrServer: быстро и без сети. `npm run smoke` запускает настоящий `bin\TorrServer.exe` во временном каталоге и проверяет, что имена настроек, которые пишет утилита, всё ещё существуют в этой сборке; без установленного бинарника он сообщает `SKIP` и выходит с нулевым кодом.
