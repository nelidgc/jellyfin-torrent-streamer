...
node .\torrent-jellyfin.mjs remove "Название" --dry-run
node .\torrent-jellyfin.mjs remove "Название"
```

`status` отвечает на вопрос «что сейчас происходит» одной таблицей: запущены ли TorrServer и шлюз, сколько пиров у каждой раздачи, сколько занято в кэше, отвечает ли ссылка и готова ли раздача к просмотру. Отметка `OK` появляется, когда есть подключённые пиры и ссылка отвечает; иначе внизу печатается причина. URL из `.strm` не выводится никогда — в нём токен, а вывод `status` обычно и попадает в сообщение с просьбой о помощи. Для скриптов есть `--json`.

`logs` читает журнал сам, поэтому кодировка не зависит от оболочки; `--errors` оставляет только предупреждения и ошибки.

`remove` удаляет раздачу целиком: сбрасывает торрент в TorrServer, убирает созданные `.strm`, чистит опустевшие папки и удаляет запись из реестра. С `--purge-cache` дополнительно удаляется каталог кэша этой раздачи. Выбирать можно по infohash, по префиксу от 6 символов или по части названия; неоднозначный запрос не выполняется, а показывает список кандидатов. Файлы `.strm`, изменённые вручную, не удаляются — как и при `rebuild`. Начинайте с `--dry-run`.
...
`doctor` проверяет Node.js, каталоги и права записи, конфигурацию TorrServer, LAN-адрес, кэш и созданные `.strm`. В конце он печатает два готовых пути для библиотек Jellyfin и предупреждает, если на `127.0.0.1:8096` никто не отвечает; это предупреждение, а не ошибка — Jellyfin может стоять на другом порту или другой машине.

Журнал пишется с BOM, поэтому `Get-Content` в Windows PowerShell 5.1 показывает нелатинские названия правильно. Журнал, созданный более ранней версией, читайте через `node .\torrent-jellyfin.mjs logs` или `Get-Content ... -Encoding utf8`.
...
### Почему тормозит и что это не

Рывки при просмотре почти всегда имеют одну из трёх причин, и различить их можно за минуту.

**Рой.** Раздача отдаёт медленнее, чем нужно фильму. В `status` это видно как малое число подключённых пиров, а в журнале — как `Torrent stream stalled` со скоростью и количеством пиров. Лечится только выбором раздачи с живыми сидами: холодная перемотка в редкую раздачу упирается в скорость роя, а не в этот проект.

**Транскодинг Jellyfin.** Если клиент не умеет декодировать дорожку (типичный случай — AC3 в браузере), Jellyfin перекодирует поток и при каждой перемотке начинает заново, с самого начала файла. Снаружи это выглядит как «проект тормозит», хотя раздача отдаёт нормально. Шлюз распознаёт это сам: несколько запросов подряд по одному файлу, каждый обрывается почти сразу и почти с нуля. Тогда в журнал попадает одно предупреждение `Playback restarted repeatedly`, а `status` показывает строку в разделе «Recent playback problems». Лечится на стороне Jellyfin: клиент, который проигрывает дорожку напрямую (Direct Play), либо другая звуковая дорожка.

**Шлюз, TorrServer и диск.** Практически никогда: отдача из кэша идёт на скорости диска. Если сомневаетесь, проверьте `curl.exe` по инструкции ниже — при попадании в кэш первые байты приходят за десятки миллисекунд.

Обычная перемотка по фильму за перезапуск не считается: срабатывание требует нескольких обрывов подряд у начала файла.
